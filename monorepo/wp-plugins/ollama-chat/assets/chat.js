(function(){
  ...
  function widget(){
    ...
    // Panel with tabs: Chat, Appointment, Upload Document
    ...
    tabs.appendChild(tabChat); tabs.appendChild(tabAppt); tabs.appendChild(tabUpload);
    ...
    const content = el('div','oc-content'); content.appendChild(chatBox); content.appendChild(apptWrap); content.appendChild(uploadWrap);
    panel.appendChild(content); root.appendChild(panel);

    function show(idx){ chatBox.style.display = idx===0?'flex':'none'; apptWrap.style.display= idx===1?'block':'none'; uploadWrap.style.display= idx===2?'block':'none';
      tabChat.classList.toggle('active', idx===0); tabAppt.classList.toggle('active', idx===1); tabUpload.classList.toggle('active', idx===2); }
    show(0);

    tabChat.onclick=()=>show(0); tabAppt.onclick=()=>show(1); tabUpload.onclick=()=>show(2);
